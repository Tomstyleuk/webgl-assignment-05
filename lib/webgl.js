...
export class WebGLUtility {
  ...
  static createVBO(gl, vertexArray) {
    ...
    // バッファを gl.ARRAY_BUFFER としてバインドする、vboをまな板に乗せて調理する
    gl.bindBuffer(gl.ARRAY_BUFFER, vbo);

    // バインドしたバッファに Float32Array オブジェクトに変換した配列を設定する
    // 32 bit - 32個の羅列というデータを作成してから渡す必要がある
    const data = new Float32Array(vertexArray);
    gl.bufferData(gl.ARRAY_BUFFER, data, gl.STATIC_DRAW);

    // 安全のために最後にバインドを解除してからバッファオブジェクトを返す、vboをまな板からどかす
